...
even with box cox transf, stationarity and any type of transf, ljox still rejects? so residuals have autocorr, not a good model

because ..... just of the time series type?

If ETS fails , Holt-Winters fails harder.

## GARCH ?

```{.r}
library(rugarch)
ts_diff <- diff(diff(ts_train, lag = 12), differences = 1)

# Specify GARCH(1,1) with ARMA(1,1) in the mean (adjust armaOrder as needed)
spec <- ugarchspec(
  variance.model = list(model = "sGARCH", garchOrder = c(1,1)),
  mean.model     = list(armaOrder = c(1,1), include.mean = FALSE),
  distribution.model = "norm"  # or "std" for Student-t, if tails are heavy
)

fit_garch <- ugarchfit(spec = spec, data = ts_diff)
show(fit_garch)
plot(fit_garch)  # volatility, residuals, standardized residuals
resid_std <- residuals(fit_garch, standardize = TRUE)

# Check for white noise
acf(resid_std)         # Should show no significant spikes
acf(resid_std^2)       # Should also be uncorrelated if GARCH fit well
forecast_garch <- ugarchforecast(fit_garch, n.ahead = 20)
plot(forecast_garch)  # Shows forecasted conditional variance
...
## OUTROS MODELOS (must always check if statin ir requeried)
